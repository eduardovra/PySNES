...
def test_generate_samples_muted():
    ...
    d.regs[0x00] = 0x7F
    d.regs[0x01] = 0x7F  # voice 0 volumes
    d.regs[0x0C] = 0x7F
    d.regs[0x1C] = 0x7F  # master volumes
...
def test_brr_end_flag_sets_endx():
    ...
    d.write_register(0x02, 0x00)
    d.write_register(0x03, 0x10)  # pitch=0x1000 (advance 1 sample/tick)
    d.write_register(0x00, 0x7F)
    d.write_register(0x01, 0x7F)
    d.write_register(0x0C, 0x7F)
    d.write_register(0x1C, 0x7F)
...
def test_brr_end_without_loop_zeroes_envelope():
    ...
    d.write_register(0x04, 0x00)  # VxSRCN=0 for voice 0
    d.write_register(0x02, 0x00)
    d.write_register(0x03, 0x10)  # pitch=0x1000
    d.write_register(0x00, 0x7F)
    d.write_register(0x01, 0x7F)
    d.write_register(0x0C, 0x7F)
    d.write_register(0x1C, 0x7F)
...
def test_key_on_activates_voice():
    ...
    mem = bytearray(65536)
    mem[0] = 0x00
    mem[1] = 0x03  # start addr = 0x0300
    mem[2] = 0x00
    mem[3] = 0x03  # loop addr = 0x0300
...
def test_generate_samples_produces_nonzero_with_active_voice():
    ...
    d.regs[0x02] = 0x00
    d.regs[0x03] = 0x00  # pitch=0 (no advance, stays at offset 0)
