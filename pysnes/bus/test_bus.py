...
def test_low_ram_write_read_bank00():
    bus, *_ = make_bus()
    bus.write(0x000100, 0xAB)
    assert bus.read(0x000100) == 0xAB


def test_low_ram_mirror_bank3f():
    """Banks $00–$3F mirror the same Low RAM."""
    bus, *_ = make_bus()
    bus.write(0x000200, 0x42)
    assert bus.read(0x3F0200) == 0x42


def test_low_ram_mirror_bank7e():
    """Bank $7E $0000–$1FFF is the canonical Low RAM."""
    bus, *_ = make_bus()
    bus.write(0x7E0500, 0x99)
    assert bus.read(0x000500) == 0x99


def test_low_ram_boundary_last_byte():
    bus, *_ = make_bus()
    bus.write(0x001FFF, 0x77)
    assert bus.read(0x001FFF) == 0x77
...
def test_high_ram_write_read():
    ...
    assert bus.read(0x7E2000) == 0x11


def test_high_ram_boundary():
    bus, *_ = make_bus()
    bus.write(0x7E7FFF, 0xCC)
    assert bus.read(0x7E7FFF) == 0xCC
...
def test_extended_ram_write_read():
    bus, *_ = make_bus()
    bus.write(0x7E8000, 0x55)
    assert bus.read(0x7E8000) == 0x55


def test_extended_ram_boundary():
    bus, *_ = make_bus()
    bus.write(0x7FFFFF, 0xEE)
    assert bus.read(0x7FFFFF) == 0xEE
...
def test_lorom_mirror_high_bank():
    ...
    assert bus.read(0x808005) == 0x12


def test_lorom_write_to_rom_region_stored():
    """Writes to ROM-mapped addresses are silently dropped (ROM is read-only on
    hardware)."""
    bus, rom, *_ = make_bus()
    bus.write(0x008100, 0x7F)
...
def test_apu_port_read_returns_ports_w():
    ...
    apu.ports_w[0] = 0xAA
    apu.ports_w[1] = 0xBB
    assert bus.read(0x002140) == 0xAA
    assert bus.read(0x002141) == 0xBB
...
def test_low_ram_boundary_first_byte():
    bus, *_ = make_bus()
    bus.write(0x000000, 0x01)
    assert bus.read(0x000000) == 0x01
...
def test_lorom_region2_read():
    ...
    bus, rom, *_ = make_bus()
    # Bank $40, addr $0010 → ROM offset: 0x40 * 0x8000 + 0x0010 = 0x200010 But
    # ROM is only 512 KB (0x80000), so use a small bank number inside range Bank
    # $40 = 64 decimal; offset = 64 * 0x8000 + 0x0010 = 0x200010 — beyond 512KB
    # Use bank $40 with addr >= 0x8000 to stay in region 1 overlap, OR use addr
    # < 0x8000 For region 2 specifically (addr 0x0000-0x7FFF in banks
    # 0x40-0x6F): rom_addr = bank * 0x8000 + addr (addr < 0x8000, so no
    # subtraction) Bank $40=64, addr $0020 → rom_addr = 64*0x8000 + 0x0020 =
    # 0x200020 (too big) Use bank $40 but stub ROM is 512KB=0x80000; 0x200020 >
...
def test_lorom_region3_read():
    ...
    bus, rom, *_ = make_bus()
    # bank $70=112, addr $8010 → rom_addr = 112*0x8000 + (0x8010-0x8000) =
    # 0x380010 0x380010 > 512KB → OOB, StubRom returns 0; test just confirms
    # routing/no crash
    val = bus.read(0x708010)
    assert val == 0
...
def test_lorom_mirror_write():
    """Write through a mirrored bank ($80+) is silently dropped (ROM is
    read-only on hardware)."""
    bus, rom, *_ = make_bus()
    bus.write(0x808010, 0x55)  # bank $80 mirrors bank $00
...
def test_joyser0_write_latches_controller():
    ...
    # After latch cycle, reading data() should return 1s (no keys pressed →
    # padding)
    val = bus.read(0x004016)  # JOYSER0 read
    assert val in range(0x100)  # sanity: valid byte returned
...
def test_joy1l_read():
    ...
    assert bus.read(0x004218) == 0xAB


def test_joy1h_read():
    bus, _, _, _, _ = make_bus()
    bus.controller_port1.joy_h = 0xCD
    assert bus.read(0x004219) == 0xCD


def test_joy2l_read():
    bus, _, _, _, _ = make_bus()
    bus.controller_port2.joy_l = 0x12
    assert bus.read(0x00421A) == 0x12


def test_joy2h_read():
    bus, _, _, _, _ = make_bus()
    bus.controller_port2.joy_h = 0x34
    assert bus.read(0x00421B) == 0x34
...
def test_memsel_fastrom_on_sets_fast_speed():
    """Writing 1 to $420D enables FastROM: banks $80-$BF/$C0-$FF use fast (6
    MC)."""
    bus, _rom, cpu, *_ = make_bus()
    bus.write(0x00420D, 0x01)  # MEMSEL: enable FastROM
...
def test_sram_basic_write_read():
    ...
    assert bus.read(0x700000) == 0xAB


def test_sram_distinct_from_wram():
    """SRAM is a separate buffer from WRAM."""
    bus, *_ = make_bus(sram_size=0x2000)
    bus.write(0x7E0100, 0x11)  # WRAM
    bus.write(0x700100, 0x22)  # SRAM (same low address, different region)
    assert bus.read(0x7E0100) == 0x11
    assert bus.read(0x700100) == 0x22


def test_sram_mirror_bank_f0():
    """Bank $F0 mirrors bank $70."""
    bus, *_ = make_bus(sram_size=0x2000)
    bus.write(0x700010, 0x55)
    assert bus.read(0xF00010) == 0x55
...
def test_sram_upper_bank_7d():
    ...
    bus.write(0x7D0000, 0x77)
    assert bus.read(0x700000) == 0x77  # same SRAM byte due to wrap
...
def test_sram_size_masking_2kb():
    ...
    bus.write(0x700000, 0xAA)
    assert bus.read(0x700800) == 0xAA
    bus.write(0x700801, 0xBB)
    assert bus.read(0x700001) == 0xBB


def test_sram_size_masking_8kb():
    """With 8KB SRAM, $0000 and $2000 alias; $0000 and $1FFF do not."""
    bus, *_ = make_bus(sram_size=0x2000)
    bus.write(0x700000, 0x01)
    bus.write(0x701FFF, 0x02)
    assert bus.read(0x700000) == 0x01
    assert bus.read(0x701FFF) == 0x02
    # Wrap: $2000 aliases $0000
    bus.write(0x702000, 0x99)
    assert bus.read(0x700000) == 0x99


def test_sram_no_sram_read_returns_open_bus():
    """When sram_size is 0, SRAM reads return 0xFF (open bus)."""
    bus, *_ = make_bus(sram_size=0)
    assert bus.read(0x700000) == 0xFF


def test_sram_no_sram_write_ignored():
    """When sram_size is 0, SRAM writes are silently ignored (no crash)."""
    bus, *_ = make_bus(sram_size=0)
    bus.write(0x700000, 0xAB)  # must not raise
    assert bus.read(0x700000) == 0xFF
...
def test_sram_roundtrip_save_load(tmp_path):
    ...
    path = tmp_path / "game.srm"
    bus, *_ = make_bus(sram_size=0x2000)
    bus.write(0x700000, 0xDE)
    bus.write(0x700001, 0xAD)
    bus.write(0x701FFF, 0xEF)
    n = bus.save_sram(str(path))
    assert n == 0x2000
    assert path.stat().st_size == 0x2000

    bus2, *_ = make_bus(sram_size=0x2000)
    assert bus2.read(0x700000) == 0x00  # fresh
    loaded = bus2.load_sram(str(path))
    assert loaded == 0x2000
    assert bus2.read(0x700000) == 0xDE
    assert bus2.read(0x700001) == 0xAD
    assert bus2.read(0x701FFF) == 0xEF
...
def test_sram_load_missing_file_noop(tmp_path):
    ...
    assert bus.read(0x700000) == 0x5A


def test_sram_load_smaller_file_preserves_tail(tmp_path):
    """Loading a file smaller than sram_size only overwrites the leading
    bytes."""
    path = tmp_path / "partial.srm"
    path.write_bytes(b"\x11\x22")
    bus, *_ = make_bus(sram_size=0x800)
    bus.write(0x700100, 0xFE)  # tail byte to preserve
    loaded = bus.load_sram(str(path))
    assert loaded == 2
    assert bus.read(0x700000) == 0x11
    assert bus.read(0x700001) == 0x22
    assert bus.read(0x700100) == 0xFE  # unchanged


def test_sram_upper_half_still_rom():
    """Bank $70 $8000-$FFFF is ROM, not SRAM — reads must not return SRAM
    data."""
...
def test_rdvraml_routes_to_ppu():
    ...
    bus.write(0x002115, 0x00)  # VMAIN: increment on low read
    bus.write(0x002116, 0x10)  # VMADDL — triggers prefetch
    bus.write(0x002117, 0x00)  # VMADDH — triggers prefetch
    assert bus.read(0x002139) == 0xAA
...
def test_rdvramh_routes_to_ppu():
    ...
    bus.write(0x002116, 0x20)
    bus.write(0x002117, 0x00)
    assert bus.read(0x00213A) == 0x22
...
def test_open_bus_unmapped_2000_range():
    """Reads in $2000-$20FF return open bus, approximated by the address high
    byte."""
    bus, *_ = make_bus()
...
def test_open_bus_unmapped_2200_range():
    """Reads in $2200-$3FFF return open bus, approximated by the address high
    byte."""
    bus, *_ = make_bus()
...
def test_hirom_read_bank_c1_spans_full_64kb():
    ...
    assert bus.read(0xC10000) == 0x11
    assert bus.read(0xC1FFFF) == 0x22


def test_hirom_read_bank_00_upper_half():
    """Bank $00 addr $8000 is the upper half of HiROM bank $C0 → rom_addr
    0x008000."""
    bus, rom, *_ = make_bus(mapping_mode=MappingMode.HIROM)
    rom.rom[0x008000] = 0xCD
    assert bus.read(0x008000) == 0xCD
...
def test_hirom_read_bank_80_mirrors_bank_00_upper_half():
    ...
    assert bus.read(0x808100) == 0xF0


def test_hirom_write_to_rom_region():
    """Writes to HiROM-mapped addresses are silently dropped (ROM is read-only
    on hardware)."""
    bus, rom, *_ = make_bus(mapping_mode=MappingMode.HIROM)
    bus.write(0xC00001, 0x7F)
    assert rom.rom[0x000001] == 0  # write ignored; ROM unchanged


def test_hirom_bank_00_low_half_is_lowram_not_rom():
    """HiROM banks $00-$3F below $8000 are system area (LowRAM / I/O), NOT
    ROM."""
    bus, rom, *_ = make_bus(mapping_mode=MappingMode.HIROM)
    bus.write(0x000100, 0xFA)
    # Must go to low_ram, not rom.rom
    assert rom.rom[0x000100] != 0xFA
    # And read-back returns the low_ram value
    assert bus.read(0x000100) == 0xFA
...
def test_hirom_bank_40_full_not_lowram():
    ...
    bus.write(0x000200, 0xDD)  # writes LowRAM (bank $00 addr $0200)
    assert bus.read(0x400200) == 0xCC  # still reads HiROM
...
def test_hirom_sram_multiple_banks():
    ...
    bus.write(0x206000, 0x44)
    bus.write(0x216000, 0x33)
    assert bus.read(0x206000) == 0x44
    assert bus.read(0x216000) == 0x33


def test_hirom_sram_size_masking_8kb():
    """With 8KB SRAM, bank $21 wraps back to bank $20."""
    bus, *_ = make_bus(sram_size=0x2000, mapping_mode=MappingMode.HIROM)
    bus.write(0x206000, 0x55)
    # sram_mask = 0x1FFF; bank $21 offset = 0x2000 & 0x1FFF = 0
    assert bus.read(0x216000) == 0x55
...
def test_hirom_sram_window_bounds():
    ...
    # aliasing SRAM.
    bus.write(0x206000, 0xAA)  # SRAM byte 0
    bus.write(0x205FFF, 0x00)
    assert bus.read(0x206000) == 0xAA
...
def test_hirom_bank_00_low_ram_still_works():
    ...
    bus.write(0x000100, 0x42)
    assert bus.read(0x000100) == 0x42
    assert bus.read(0x7E0100) == 0x42  # LowRAM canonical
...
def test_hirom_bank_00_hw_registers_still_work():
    ...
    bus.write(0x002100, 0x0F)  # INIDISP, display enable
    assert not ppu.display_disable
...
def test_multiply_basic():
    """Writing WRMPYB ($4203) triggers 8x8 unsigned multiply; result in
    $4216-$4217."""
    bus, *_ = make_bus()
...
def test_divide_basic():
    """Writing WRDIVB ($4206) triggers 16÷8 unsigned divide; quotient in
    $4214-$4215, remainder in $4216-$4217."""
    bus, *_ = make_bus()
...
def test_divide_high_dividend():
    ...
    bus, *_ = make_bus()
    bus.write(0x004205, 1)  # high byte → dividend = 256
    bus.write(0x004204, 0)  # low byte
    bus.write(0x004206, 16)  # divisor
    assert bus.read(0x004214) == 16
    assert bus.read(0x004215) == 0
    assert bus.read(0x004216) == 0
    assert bus.read(0x004217) == 0
...
def test_divide_by_zero():
    ...
    bus.write(0x004206, 0)  # divide by zero
    assert bus.read(0x004214) == 0xFF
    assert bus.read(0x004215) == 0xFF
    assert bus.read(0x004216) == 0x34  # remainder = dividend low
    assert bus.read(0x004217) == 0x12  # remainder = dividend high
