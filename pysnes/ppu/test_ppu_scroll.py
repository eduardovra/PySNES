...
class TestHorizontalScroll:
    ...
    def test_horizontal_wrap(self):
        ...
        assert _pixel(ppu, 0, 0) == BLUE, (
            "wrapping: tile 31 (BLUE) at left edge"
        )
...
class TestTilemapWordBits:
    ...
    def test_palette0_renders_red(self):
        ...
        _draw_row(ppu, scanline=1)
        assert _pixel(ppu, 0, 0) == (255, 0, 0), (
            "palette 0 color 1 should be RED"
        )
    ...
    def test_palette1_renders_green(self):
        ...
        _draw_row(ppu, scanline=1)
        assert _pixel(ppu, 0, 0) == (0, 255, 0), (
            "palette 1 color 1 should be GREEN"
        )
    ...
    def test_priority0_renders_on_low_pass(self):
        ...
        _draw_row(ppu, scanline=1)
        assert _pixel(ppu, 0, 0) == (255, 0, 0), (
            "priority=0 tile should render on low pass"
        )
    ...
    def test_priority1_renders_on_high_pass(self):
        ...
        _draw_row(ppu, scanline=1)
        assert _pixel(ppu, 0, 0) == (0, 0, 0), (
            "priority=1 tile must not render on low pass"
        )

        # High-priority pass: SHOULD render
        bg_renderer.draw_background_scanline(
            ppu, ppu.bg1, bpp=2, priority_selector=1
        )
        assert _pixel(ppu, 0, 0) == (255, 0, 0), (
            "priority=1 tile should render on high pass"
        )
