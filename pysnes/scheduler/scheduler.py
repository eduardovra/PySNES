...
class Scheduler:
    """
    Discrete Event Scheduler (DES) — the single source of time for all
    components.
    ...
    Timing reference:
        ...
        - 1 APU clock                 ≈ 21 master clocks
          (exact ratio: 21477272/1024000)
    """
    ...
    def run_to(self, target: int) -> None:
        """Fire all events whose time <= target, advancing master_clock to
        each."""
        q = self._queue
    ...
    def dump_state(self) -> dict:
        ...
        for t, seq, fn in self._queue:
            ...
            if owner is None or method is None:
                raise ValueError(
                    "Cannot serialize scheduler entry with non-bound-method "
                    f"handler: {fn!r}"
                )
    ...
    def load_state(self, d: dict, registry: dict) -> None:
        ...
        instance, e.g. {"cpu": pysnes.cpu, "ppu": pysnes.ppu}.
        """
        ...
        for t, seq, owner_kind, method_name in d["queue"]:
            ...
            if not callable(fn):
                raise ValueError(
                    f"Scheduler load: {owner_kind}.{method_name} is not "
                    "callable"
                )
