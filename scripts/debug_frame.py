...
def _patched_get(self, abs_addr):
    ...
    if 0x2140 <= addr <= 0x2143:
        port_log.append(f'R ${addr:04X}={hex(val)} ports_w[{addr-0x2140}]  CPU_PC={hex(cpu.PC.value)} A.l={hex(cpu.A.l)} MC={scheduler.master_clock}')
    return val
_orig_set = bus.__class__.__setitem__
def _patched_set(self, abs_addr, data):
    addr = abs_addr & 0xFFFF
    if 0x2140 <= addr <= 0x2143:
        port_log.append(f'W ${addr:04X}={hex(data)}              CPU_PC={hex(cpu.PC.value)} A.l={hex(cpu.A.l)} MC={scheduler.master_clock}')
...
for i, entry in enumerate(port_log):
    if 'R $2140=' in entry and 'A.l=0x0 ' in entry and 'R $2140=0x0 ' not in entry:
        stuck = i
        break
...
print()
print(f'CPU PC={hex(cpu.PC.value)}  A={hex(cpu.A.value)}  A.l={hex(cpu.A.l)}  P={hex(cpu.P):4s}')
print(f'apu.ports_r={list(apu.ports_r)}  apu.ports_w={list(apu.ports_w)}  apu.PC={hex(apu.PC)}')
