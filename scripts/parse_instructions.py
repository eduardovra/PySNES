...
for line in filter(None, lines):
    line = line.strip()
    m = re.match(r"(op[AMX])\((.*)\)$", line)
    addr_mode = m.group(1)
    elems = m.group(2).split(', ')
    #print(addr_mode, elems)
    opcode = elems[0]
    args = [opcode]
    addr_mode_func = elems[1]
    if addr_mode == 'opM':
        addr_mode_func += '.MF'
    elif addr_mode == 'opX':
        addr_mode_func += '.XF'
    args.append(f'AM.{addr_mode_func}')

    for e in elems[2:]:
        # opcdes
        m = re.match(r"([mx])\((.*)\)", e)
        if m:
            assert f'op{m.group(1).upper()}' == addr_mode
            opcode_func = m.group(2)
            args.append(f'OP.{opcode_func}')
            continue
        ...
        if m:
            if m.group(2) == '1':
                args.append(f'lambda cpu: cpu.{m.group(1)}')
            else:
                args.append(f'lambda cpu: not cpu.{m.group(1)}')
            continue
